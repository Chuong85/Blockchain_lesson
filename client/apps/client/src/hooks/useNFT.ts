...
export const useMutateBuyNFT = (contractAddress: string, tokenId: string, price: string) => {
  const { data: walletClient } = useWalletClient()
  const publicClient = usePublicClient()
  const queryClient = useQueryClient()
  ...
  return useMutation({
    mutationFn: async () => {
      ...
      const tx = await walletClient.sendTransaction({
        to: CONTRACT_ADDRESSES.MARKETPLACE_ADDRESS as `0x${string}`,
        data: contract.interface.encodeFunctionData('buyNFT', [contractAddress, tokenId]) as `0x${string}`,
